...
/**
 * The golden vectors are the only thing keeping this copy of the encoder honest.
 *
 * `packages/hex` may not import from grimoire's `src/`, so `src/nostr/` holds a
 * duplicate of the wire library. Both trees run this file against the same
 * fixture: if either copy changes shape, the ids diverge and one suite fails.
 */
